...
namespace UMol
{
    ...
    public class PDBxReader : Reader
    {
        ...
        /// </summary>
        public IEnumerator Fetch(string entryCode, System.Action<UnityMolStructure> result, bool readHet = true,
            bool readWater = true)
        {
            ...
            bool simplyParse = false)
        {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if ( splits.Length > 0 )
                    {
                        ...
                        if ( readAtomLine )
                        {
                            ...
                            float xcoord = -1*float.Parse(splitAtomLine[atomKeyId["Cartn_x"]],
                                System.Globalization.CultureInfo.InvariantCulture);
                            float ycoord = float.Parse(splitAtomLine[atomKeyId["Cartn_y"]],
                                System.Globalization.CultureInfo.InvariantCulture);
                            float zcoord = float.Parse(splitAtomLine[atomKeyId["Cartn_z"]],
                                System.Globalization.CultureInfo.InvariantCulture);
                            Vector3 coord = new Vector3(xcoord, ycoord, zcoord);

                            if ( minimalFields )
                            {
                                UnityMolAtom curAtom = new UnityMolAtom(fullAtomElement + "_" + atomSerial, atomElement,
                                    coord, 0.0f, atomSerial, true);
                            ...
                            float bfactor = float.Parse(splitAtomLine[atomKeyId["B_iso_or_equiv"]],
                                System.Globalization.CultureInfo.InvariantCulture);
                                    ...
                                    if ( resNum == lastResidueId )
                                    {
                                        ...
                                        if ( resName != lastResidue )
                                        {
                                            ...
                                            debug.Append(String.Format(
                                                "Residue number {0} on chain {1} defined multiple times\n", resNum,
                                                atomChain));
                                            continue;
                                        }
                                        else
                            ...
                            UnityMolAtom newAtom = new UnityMolAtom(atomName, atomElement, coord, bfactor, atomSerial,
                                isHetAtm);
                        ...
                        if ( QuickStartWith(trimmedLine, keyFieldHelix) )
                        {
                            ...
                            string[] sl = trimmedLine.Split(new[] { '\t', ' ' },
                                System.StringSplitOptions.RemoveEmptyEntries);
                            //Not loop version
                            if ( sl.Length == 2 )
                            {
                            ...
                            {
                                string n = trimmedLine.Substring(trimmedLine.IndexOf(keyFieldHelix) +
                        ...
                        if ( QuickStartWith(trimmedLine, keyFieldSheet) )
                        {
                            ...
                            {
                                string n = trimmedLine.Substring(trimmedLine.IndexOf(keyFieldSheet) +
                        ...
                        if ( QuickStartWith(trimmedLine, keyFieldConnec) )
                        {
                            ...
                            {
                                string n = trimmedLine.Substring(trimmedLine.IndexOf(keyFieldConnec) +
            ...
            if ( newStruct.models.Count != 1 )
            {
                ...
                {
                    CreateColliders(new UnityMolSelection(newStruct.models[i].allAtoms, null, sel.name,
