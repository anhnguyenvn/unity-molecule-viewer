    ...
    public class SurfaceThread
    {
        ...
        void ComputeMSMSSurface()
        {
            ...
            foreach (UnityMolSelection sub in subSels)
            {
                ...
                string keyPrecomputedRep = sub.atoms[0].residue.chain.model.structure.uniqueName + "_" +
                                           sub.atoms[0].residue.chain.name + "_" + SurfMethod.MSMS.ToString();
        ...
        void ComputeEDTSurface()
        {
            ...
            foreach (UnityMolSelection sub in subSels)
            {
                ...
                //Don't compute the chain if already there
                string keyPrecomputedRep = sub.atoms[0].residue.chain.model.structure.uniqueName + "_" +
                                           sub.atoms[0].residue.chain.name + "_" + SurfMethod.EDTSurf.ToString();
                if ( UnityMolMain.getPrecompRepManager().precomputedRep.ContainsKey(keyPrecomputedRep) )
                {
                    continue;
                }
                ...
                int[] meshAtomVert = null;
                MeshData tmp =
                    EDTSurfWrapper.callEDTSurf(sel.name + "_" + idSub.ToString(), pdbLines, ref meshAtomVert);
                if ( tmp != null )
                {
