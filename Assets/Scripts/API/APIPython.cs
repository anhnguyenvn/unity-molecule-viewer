    ...
    namespace API
    {
        public class APIPython : MonoBehaviour
        {
            ...
            public static UnityMolStructure load(string filePath, bool readHetm = true, bool forceDSSP = false,
                bool showDefaultRep = true, bool center = true, bool modelsAsTraj = true)
            {
            ...
            /// </summary>
            public static UnityMolStructure loadFromString(string fileName, string fileContent, bool readHetm = true,
                bool forceDSSP = false, bool showDefaultRep = true, bool center = true, bool modelsAsTraj = true)
            {
                UnityMolStructure newStruct = null;
            ...
            /// </summary>
            public static UnityMolStructure fetch(string PDBId, bool usemmCIF = true, bool readHetm = true,
                bool forceDSSP = false, bool showDefaultRep = true, bool center = true, bool modelsAsTraj = true)
            {
                UnityMolStructure newStruct = null;
            ...
            public static void show(string type)
            {
                ...
                if ( repType.atomType != AtomType.noatom || repType.bondType != BondType.nobond )
                {
                    foreach (UnityMolStructure s in sm.loadedStructures)
                    {
                        List<UnityMolRepresentation> existingReps =
                            repManager.representationExists(s.ToSelectionName(), repType);
            ...
            public static bool defaultRep(string selName)
            {
                ...
                if ( selM.selections.ContainsKey(selName) )
                {
                    ...
                    if ( sel.structures != null && sel.structures.Count > 0 )
                    {
                        ...
                        MDAnalysisSelection selecUnreco =
                            new MDAnalysisSelection("not protein and not water and not ligand and not ions", sel.atoms);
            ...
            public static void showSelection(string selName, string type, params object[] args)
            {
                    ...
                    if ( repType.atomType != AtomType.noatom || repType.bondType != BondType.nobond )
                    {
                        ...
                        if ( existingReps != null )
                        {
                            foreach (UnityMolRepresentation existingRep in existingReps)
                            {
                                ...
                                else if ( repType.bondType != BondType.nobond &&
                                          sel.bonds.Count != existingRep.nbBondsInRep )
                                {
                                    existingRep.updateWithNewSelection(sel);
                                }
                ...
                foreach (object o in args)
                {
                    ...
                    {
                        command += ", (DXReader)UnityMolMain.getSelectionManager().selections[\"" + selName +
                                   "\"].structures[0].dxr";
                    }
                    else
                    {
                ...
                if ( selM.selections.ContainsKey(selName) )
                {
                    ...
                    if ( existingReps != null )
                    {
                        ...
                        foreach (UnityMolRepresentation existingRep in existingReps)
                        {
                            foreach (SubRepresentation sr in existingRep.subReps)
                            {
                                UnityMolAtomBondOrderManager
                                    hbManager = (UnityMolAtomBondOrderManager)sr.atomRepManager;
                                UnityMolBondBondOrderManager
                                    hsManager = (UnityMolBondBondOrderManager)sr.bondRepManager;
                                hbManager.SetTexture(tex);
                                hsManager.SetTexture(tex);
                            }
                        }
            ...
            /// </summary>
            public static void colorByCharge(string selName, bool normalizeDensity = false, float minDens = -10.0f,
                float maxDens = 10.0f)
            {
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
                UnityMolRepresentationManager repManager = UnityMolMain.getRepresentationManager();
                ...
                if ( selM.selections.ContainsKey(selName) )
                {
                    ...
                    if ( existingReps != null )
                    {
                        foreach (UnityMolRepresentation existingRep in existingReps)
                        {
                            ...
                            {
                                UnityMolSurfaceManager surfM = (UnityMolSurfaceManager)sr.atomRepManager;
                                surfM.ColorByCharge(normalizeDensity, minDens,
                                    maxDens); //Use default -10 | 10 range to show charge
                            }
                        }
                    }
            ...
            /// </summary>
            public static UnityMolSelection select(string selMDA, string name = "selection",
                bool createSelection = true, bool addToExisting = false, bool silent = false,
                bool setAsCurrentSelection = true, bool forceCreate = false, bool allModels = false,
                bool addToSelectionKeyword = true)
            {
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
                ...
                UnityMolSelection result =
                    new UnityMolSelection(new List<UnityMolAtom>(), newBonds: null, name, selMDA);
            ...
            /// </summary>
            public static void addToSelection(string selMDA, string name = "selection", bool silent = false,
            ...
            /// </summary>
            public static void removeFromSelection(string selMDA, string name = "selection", bool silent = false,
                bool allModels = false)
            {
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
                UnityMolSelectionManager selM = UnityMolMain.getSelectionManager();
            ...
            public static UnityMolSelection selectInSphere(Vector3 position, float radius)
            {
                ...
                foreach (UnityMolStructure s in sm.loadedStructures)
                {
                    ...
                    string selMDA = "insphere " + sphereCenter.x + " " + sphereCenter.y + " " + sphereCenter.z + " " +
                                    sphereRadius.ToString("F3");
            ...
            public static void updateRepresentations(string selName)
            {
                ...
                if ( !selM.selections.ContainsKey(selName) )
                {
                    Debug.LogError("Cannot update representations of the selection '" + selName +
            ...
            static string loadedMolParentToString()
            {
                ...
                foreach (UnityMolStructure s in sm.loadedStructures)
                {
                    ...
                    res += "\n\nsetStructurePositionRotation(\"" + s.uniqueName + "\", Vector3(" +
                           spos.x.ToString("F4", culture) + ", " +
                ...
                res += "\n#Save parent position\nsetMolParentTransform( Vector3(" +
                       parentT.position.x.ToString("F4", culture) + ", " +
                       parentT.position.y.ToString("F4", culture) + ", " + parentT.position.z.ToString("F4", culture) +
                       "), Vector3(" + parentT.localScale.x.ToString("F4", culture) + ", " +
                       parentT.localScale.y.ToString("F4", culture) + ", " +
                       parentT.localScale.z.ToString("F4", culture) + "), Vector3(" +
            ...
            IEnumerator delayedSetTransform(Transform t, Vector3 pos, Vector3 scale, Vector3 rot,
            ...
            public static void removeAnnotationAngle(string structureName, int atomId, string structureName2,
                int atomId2, string structureName3, int atomId3)
            {
                UnityMolAnnotationManager anM = UnityMolMain.getAnnotationManager();
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
            ...
            public static void annotateDihedralAngle(string structureName, int atomId, string structureName2,
                int atomId2, string structureName3, int atomId3, string structureName4, int atomId4)
            {
                UnityMolAnnotationManager anM = UnityMolMain.getAnnotationManager();
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
            ...
            public static void annotateRotatingArrow(string structureName, int atomId, string structureName2,
                int atomId2)
            {
                UnityMolAnnotationManager anM = UnityMolMain.getAnnotationManager();
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
            ...
            public static void annotateArcLine(string structureName, int atomId, string structureName2, int atomId2,
                string structureName3, int atomId3)
            {
                UnityMolAnnotationManager anM = UnityMolMain.getAnnotationManager();
                UnityMolStructureManager sm = UnityMolMain.getStructureManager();
