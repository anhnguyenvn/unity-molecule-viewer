...
//
    public class XDRFileReader
    {
        ...
        public int open_trajectory(UnityMolStructure stru, string filename, bool is_trr = false)
        {
            ...
            {
                Debug.LogWarning("Trajectory has not the same number of atoms than the first model of the structure." +
                                 numberAtoms + " vs " + structure.Count);
//				numberAtoms = (int)MoleculeModel.atomsnumber;
        ...
        private Vector3[] loadOneFrame(int frame_number)
        {
            ...
            for (int i = 0; i < numberAtoms; i++)
            {
                ...
                trajectoryBuffer[idB][i] = new Vector3(-trajectoryBufferF[i*3], trajectoryBufferF[i*3 + 1],
                    trajectoryBufferF[i*3 + 2])*10.0f;
            }
...
// The array of positions is required to have a size == sizeof(float) * numberAtoms * 3 for this to work properly.
        public int next_frame(ref int step, ref float time, [In, Out] float[] positions, ref float precision)
        {
            ...
            if ( is_trr )
            {
                float lambda = 0.0f;
                int res = 0;
                res = (int)XDRFileWrapper.read_trr(file_pointer, numberAtoms, ref step, ref time, ref lambda, box,
                    positions, null, null);
                return res;
            }
            else
            {
                return (int)XDRFileWrapper.read_xtc(file_pointer, numberAtoms, ref step, ref time, box, positions,
                    ref precision);
            }
        }
