...
namespace UMol
{
    public class ArtemisThreadedClient
    {
                    ...
                    switch (header.type)
                    {
                        ...
                        case ArtemisWrapper.artemis_imd_type_e.IMD_FCOORDS:
                            ...
                            if ( ArtemisWrapper.artemis_client_read_coords(client, pos2, capacity) == 1 )
                            {
                                ...
                                {
                                    if ( float.IsNaN(pos2[i*3]) || float.IsInfinity(pos2[i*3]) ||
                                         Mathf.Abs(pos2[i*3]) > 10000.0f )
                                    {
                                        artemisM.positions[i] = Vector3.one*-100.0f;
                                        continue;
                                    }

                                    if ( float.IsNaN(pos2[i*3 + 1]) || float.IsInfinity(pos2[i*3 + 1]) ||
                                         Mathf.Abs(pos2[i*3 + 1]) > 10000.0f )
                                    {
                                        artemisM.positions[i] = Vector3.one*-100.0f;
                                        continue;
                                    }

                                    if ( float.IsNaN(pos2[i*3 + 2]) || float.IsInfinity(pos2[i*3 + 2]) ||
                                         Mathf.Abs(pos2[i*3 + 2]) > 10000.0f )
                                    {
                                        artemisM.positions[i] = Vector3.one*-100.0f;
                                        continue;
                                    }
        ...
        public int send_position_restraint(ArtemisClientPointer client,
            ArtemisWrapper.ArtemisEimdPositionRestraint position_restraint)
        {
    ...
    public class ArtemisManager : MonoBehaviour
    {
        ...
        public bool connect(string host, int port)
        {
            ...
            if ( res == 0 )
            {
                ...
                if ( protocol_version != ArtemisWrapper.ARTEMIS_IMD_VERSION &&
                     protocol_version != ArtemisWrapper.ARTEMIS_EIMD_VERSION )
                {
