...
namespace UMol
{
    public class CartoonRepresentation : AtomRepresentation
    {
        ...
        public CartoonRepresentation(string structName, UnityMolSelection sel, bool useHetatm = false,
            bool useWat = false)
        {
        ...
        List<Segment> cutSelectionInSegments(UnityMolSelection sele)
        {
            ...
            for (int i = 0; i < sele.atoms.Count; i++)
            {
                ...
                if ( !useWAT &&
                     WaterSelection.waterResidues.Contains(atom.residue.name, StringComparer.OrdinalIgnoreCase) )
                {
                    continue;
                }
        ...
        private void removeMeshParts(UnityMolSelection bigSel)
        {
            ...
            {
                if ( !useWAT &&
                     WaterSelection.waterResidues.Contains(a.residue.name, StringComparer.OrdinalIgnoreCase) )
                {
                    continue;
                }
        ...
        public void displayCartoonMesh(string structName, List<Segment> segments, Transform repParent,
            ...
            float bRad = 0.3f, int bRes = 10, bool useBspline = true, bool isTraj = false)
        {
            ...
            for (int seg = 0; seg < segments.Count; seg++)
            {
                ...
                if ( UnityMolMain.getPrecompRepManager().ContainsRep(keyPrecomputedRep) )
                {
                    ...
                    Dictionary<UnityMolResidue, List<int>> tmpResVert =
                        UnityMolMain.getPrecompRepManager().precomputedCartoonAsso[keyPrecomputedRep];
